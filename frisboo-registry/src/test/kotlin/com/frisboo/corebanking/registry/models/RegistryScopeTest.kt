...
internal class RegistryScopeTest :
    StringSpec({
        ...
        "accepts max-length 63-char identifier" {
            ...
            shouldNotThrow<IllegalArgumentException> {
                RegistryScope(name = maxName, team = "t")
            }
        }

        "accepts hyphens in middle" {
            shouldNotThrow<IllegalArgumentException> {
                RegistryScope(name = "my-scope", team = "my-team")
            }
        }

        "accepts digits in middle and end" {
            shouldNotThrow<IllegalArgumentException> {
                RegistryScope(name = "scope1", team = "team2")
            }
        }

        "rejects empty name" {
            shouldThrow<IllegalArgumentException> {
                RegistryScope(name = "", team = "team")
            }
        }

        "rejects name starting with digit" {
            shouldThrow<IllegalArgumentException> {
                RegistryScope(name = "1scope", team = "team")
            }
        }

        "rejects name starting with hyphen" {
            shouldThrow<IllegalArgumentException> {
                RegistryScope(name = "-scope", team = "team")
            }
        }
        ...
        "rejects 64-char identifier (exceeds max)" {
            ...
            shouldThrow<IllegalArgumentException> {
                RegistryScope(name = tooLong, team = "team")
            }
        }

        "rejects special characters (colon for Redis injection)" {
            shouldThrow<IllegalArgumentException> {
                RegistryScope(name = "scope:inject", team = "team")
            }
        }
        ...
        "rejects underscore" {
            shouldThrow<IllegalArgumentException> {
                RegistryScope(name = "my_scope", team = "team")
            }
        }

        "team validation uses same rules as name" {
            shouldThrow<IllegalArgumentException> {
                RegistryScope(name = "valid", team = "INVALID")
            }
        }
