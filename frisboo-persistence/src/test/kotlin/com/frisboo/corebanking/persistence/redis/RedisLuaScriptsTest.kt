...
class RedisLuaScriptsTest {
    companion object {
        ...
        private lateinit var coroutines: RedisCoroutinesCommands<ByteArray, ByteArray>
        ...
        fun setup() {
            ...
            assumeTrue(System.getenv("RUN_INTEGRATION_TESTS") == "true", "Integration tests disabled; set RUN_INTEGRATION_TESTS=true to enable")
    ...
    fun `compareAndSet sets when key absent and expected null`() = runBlocking {
        ...
        val result = coroutines.eval<Long>(
            ...
            arrayOf(key),
            arrayOf(expected, value, "__NULL__".toByteArray())
        )
