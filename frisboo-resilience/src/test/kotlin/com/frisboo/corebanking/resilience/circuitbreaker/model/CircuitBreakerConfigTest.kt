...
internal class CircuitBreakerConfigTest : StringSpec(
    {
        ...
        "accepts minimumNumberOfCalls equal to slidingWindowSize" {
            shouldNotThrow<IllegalArgumentException> {
                validBaseConfig().copy(slidingWindowSize = 10, minimumNumberOfCalls = 10)
            }
        }

        "accepts zero maxWaitDurationInHalfOpenState" {
            shouldNotThrow<IllegalArgumentException> {
                validBaseConfig().copy(maxWaitDurationInHalfOpenState = Duration.ZERO)
            }
        }

        "accepts both sliding window types" {
            shouldNotThrow<IllegalArgumentException> {
                validBaseConfig().copy(slidingWindowType = SlidingWindowType.COUNT_BASED)
            }
            shouldNotThrow<IllegalArgumentException> {
                validBaseConfig().copy(slidingWindowType = SlidingWindowType.TIME_BASED)
            }
        }

        "accepts writableStackTraceEnabled true and false" {
            shouldNotThrow<IllegalArgumentException> {
                validBaseConfig().copy(writableStackTraceEnabled = true)
            }
            shouldNotThrow<IllegalArgumentException> {
                validBaseConfig().copy(writableStackTraceEnabled = false)
            }
        }
        ...
        "accepts empty recordExceptions and ignoreExceptions" {
            shouldNotThrow<IllegalArgumentException> {
                validBaseConfig().copy(recordExceptions = emptySet(), ignoreExceptions = emptySet())
            }
        }
    },
)
